...
export default async function updateRatings(winnerId: number | null, player1Id: number, player2Id: number, k: number = 32) {
    ...
    const player2Expected = 1 / (1 + Math.pow(10, (player1.rating - player2.rating) / 400));
    ...
    const player2NewRating = player2.rating + k * ((!winnerId ? 0.5 : !player1Winner ? 1 : 0) - player2Expected);
